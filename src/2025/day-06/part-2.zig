...
fn run(gpa: Allocator, io: Io, input: []const u8) !usize {
    ...
    while (cursor < op_line.len - 1) {
        ...
        var op_res = try parseCol(num_matrix, cursor);
        for (cursor + 1..end + 1) |idx| {
            const num = try parseCol(num_matrix, idx);
            switch (op) {
                .add => op_res += num,
                .mult => op_res *= num,
            }
...
fn parseCol(num_matrix: []const []const u8, col_idx: usize) !usize {
    ...
    for (num_matrix) |line| {
        ...
        found = true;
        res = res * 10 + try fmt.charToDigit(char, 10);
    }
    if (!found) return error.NoNumFound;
