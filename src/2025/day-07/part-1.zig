...
fn run(gpa: Allocator, io: Io, input: []const u8) !usize {
    ...
    var beam_cols: std.AutoArrayHashMapUnmanaged(usize, void) = .empty;
    ...
    try beam_cols.put(gpa, first_beam_col, {});
    ...
    while (iter.next()) |full_line| {
        ...
        for (beams_to_split.items) |beam_col| {
            if (beam_col > 0)
                try beam_cols.put(gpa, beam_col - 1, {});
